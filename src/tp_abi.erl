...
-export([find_function/2, find_event/2, events/1, mk_sig/1]).
-export([decode_abi/2]).
...
mk_sig({{EventOrFunction,Name},CS,_}) when EventOrFunction == event;
  ...
  list_to_binary([
                  ...
                  "(",
                  lists:join(",",
                             [ atom_to_binary(Type,utf8) || {_,Type} <- CS ]
                            ),
                  ")"
                 ]).
...
parse_item(#{}=_Any) ->
  %logger:notice("Unknown item: ~p~n",[_Any]),
  false.
...
encode_typed(RVal,[{_Name,{tuple,List}}|RType], Hdr, Body, BOff) ->
  %todo check if multiple some values left
  EncStr=encode_typed(RVal, List, Hdr, Body, BOff),
  encode_typed([], %probably incorrect
               RType,
